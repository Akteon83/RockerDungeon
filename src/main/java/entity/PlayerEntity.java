...
public class PlayerEntity extends LivingEntity {
    ...
    public void update(Point mousePosition) {
        ...
        angle = Math.acos((mousePosition.x - panel.screenCenter.x) / panel.screenCenter.distance(mousePosition)) *
                Integer.signum(mousePosition.y - panel.screenCenter.y);
    ...
    public void pickInstrument(InstrumentEntity instrumentEntity) {
        fireCharge = 0;
        if (instrument != null) panel.instrumentEntities.set(panel.instrumentEntities.indexOf(instrumentEntity),
                new InstrumentEntity(instrumentEntity.x, instrumentEntity.y, instrument, panel));
        else panel.instrumentEntities.remove(instrumentEntity);
        instrument = instrumentEntity.instrument;
    }

    public void throwInstrument() {
        fireCharge = 0;
        panel.instrumentEntities.add(new InstrumentEntity(getCenterX() + (32 * Math.cos(angle) - 8) * GamePanel.SIZE,
                getCenterY() + (32 * Math.sin(angle) - 16) * GamePanel.SIZE, instrument, panel));
        instrument = null;
    }
    ...
    private void move() {
        ...
        int dx = panel.keyHandler.dx;
        int dy = panel.keyHandler.dy;
